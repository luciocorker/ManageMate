-- Complete messaging schema fix
-- This will add all missing columns and create missing tables

-- Step 1: Fix channel_members table - add missing columns
DO $$ 
BEGIN
  ...
  IF NOT EXISTS (
    ...
    ADD COLUMN channel_id UUID REFERENCES public.channels(id) ON DELETE CASCADE NOT NULL;
...
-- Add unique constraint to channel_members
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Step 2: Create direct_messages table if it doesn't exist
...
-- Step 3: Add missing columns to channels table
DO $$ 
BEGIN
...
-- Step 4: Add missing columns to messages table
DO $$ 
BEGIN
  -- Add channel_id if missing
  IF NOT EXISTS (
...
-- Show channels columns
SELECT 'Channels columns:' as info;
SELECT column_name, data_type
FROM information_schema.columns
WHERE table_schema = 'public' AND table_name = 'channels'
ORDER BY ordinal_position;

-- Show channel_members columns
SELECT 'Channel members columns:' as info;
SELECT column_name, data_type
FROM information_schema.columns
